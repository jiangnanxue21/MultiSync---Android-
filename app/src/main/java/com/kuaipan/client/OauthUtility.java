...
public class OauthUtility {	
	...
	private static KuaipanURL buildURL(String method, String host, String location, 
			...
			Consumer consumer, TokenPair token,
			boolean isSecure) {
		return buildURL(method, host, location, params, consumer, token, 10, isSecure);
	}
		
	public static KuaipanURL buildURL(String method, String host, String location, 
			Map<String, String> params,
			Consumer consumer, TokenPair token,
			int nonceLength,
			boolean isSecure) {
		...
		signed_params.put("oauth_nonce", generateNonce(nonceLength));
